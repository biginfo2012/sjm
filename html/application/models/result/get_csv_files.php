	...
	public function print_crm($value='')
	{
		if($this->csv_type['crm'] == 1){
			...
			// Districtコード	District名称	営業員コード	営業員氏名	訪問日時	病院コード	病院名称	タスクタイトル	訪問相手	訪問内容（複数列方向に出てくるはずです）	立会い内容	その他（フリーコメント)
				...
				$line[] = $val['worker_name'];
				$line[] = $val['visit_schedule'];
				$line[] = substr($val['visit_time_from'], 0 , 2) . ':' . substr($val['visit_time_from'], 2);
				$line[] = substr($val['visit_time_to'], 0 , 2) . ':' .substr($val['visit_time_to'], 2);
				$line[] = $val['sap_no'];
				$line[] = $val['facility_name'];
				$visit_valid_type = "";
				if($val['visit_valid_type'] == 1){
				 		$visit_valid_type = "有効面談";
				}
				$line[] = $visit_valid_type;
