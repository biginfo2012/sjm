...
$(document).ready(function(){
  ...
  $('#start-month').on('change', function () {
    start_date = $(this).val()
    if(start_date !== ''){
      if(end_date !== ''){
        enable_search = true;
        var start_month = '&start_month=' + start_date;
        var end_month = '&end_month=' + end_date;
        var href = $('#search_period')[0].href;
        href = href + start_month + end_month;
        $('#search_period').removeClass('disabled');
        $('#search_period').prop('href', href);
        console.log(href)
      }else{
        $('#search_period').addClass('disabled');
      }
    }
    else{
      $('#search_period').addClass('disabled');
...
$("a.m1, button.m1, button.m1-sp").click(function(){
  ...
  if($('html').hasClass('open-1')){
	...
	$('#visit_valid_type').prop('checked', false);
    $('#report_remarks').val('');
    ...
    $.ajax({
      ...
      success: function( result ) {
        ...
        if(schedule_id != ''){
		  ...
		  if(result['visit_valid_type'] == 1){
		  	$('#visit_valid_type').prop('checked', true);
		  }
